...
uint8_t DS18X20_meas_to_cel(uint8_t fc, uint8_t *sp, uint8_t *subzero, uint8_t *cel, uint8_t *cel_frac_bits)
{
    ...
    if (meas & 0x8000) {
        *subzero = 1;     // Mark negative
        meas ^= 0xffff;   // Convert to positive => (two's complement)++
        meas++;
    } else {
        *subzero = 0;
    }

    // Clear undefined bits for DS18B20 != 12-bit resolution
    if (fc == DS18B20_ID) { // Check resolution for DS18B20
        i = sp[DS18B20_CONF_REG];
        if ((i & DS18B20_12_BIT) == DS18B20_12_BIT) {
            // No action needed for 12-bit resolution
        } else if ((i & DS18B20_11_BIT) == DS18B20_11_BIT) {
            meas &= ~(DS18B20_11_BIT_UNDF);
        } else if ((i & DS18B20_10_BIT) == DS18B20_10_BIT) {
            meas &= ~(DS18B20_10_BIT_UNDF);
        } else { // 9-bit resolution
